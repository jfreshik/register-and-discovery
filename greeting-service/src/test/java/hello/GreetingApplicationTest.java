...
public class GreetingApplicationTest {
    ...
    public void shouldRegisterClientInEurekaServer() throws InterruptedException {
        ...
        final String applicationName = "greeting-service";

        // registration has to take place...
        Thread.sleep(3000);

        ResponseEntity<String> response = this.testRestTemplate.getForEntity("http://localhost:" + this.port + "/service-instances/" + applicationName, String.class);

        then(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        then(response.getBody()).contains(applicationName);
    }
